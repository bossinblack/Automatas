    ...
    for i in list(cadena):
        if state_a == 1:                            #si estoy en el estado a
            ...
            if i == 'a' or i == 'A': #y entra una a
                print("Con %s Pasa a estado D" % i)
                state_d = 1
                state_e = state_b = state_c = state_a = 0
            elif i == 'b' or i == 'B':#si entra una b
                print("Con %s Vacio: queda en estado C" % i)
                break
        elif state_d == 1:  # si stoy en d
            if i == 'a' or i == 'A':  # y entra una a
                print("Con %s Vacio: queda en estado D" % i)
                break
            elif i == 'b' or i == 'B':  # o si entra una b
                print("Con %s Pasa a estado E" % i)
                state_e = 1
                state_b = state_d = state_c = state_a = 0
        elif state_e == 1:               #si stoy en el estado e
            if i == 'b' or i == 'B':  # o si entra una b
                print("Con %s Pasa a estado de finalizacion F" % i)
            elif i == 'a' or i == 'A':  #si entra una a
